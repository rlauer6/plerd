...
If you've forked my version of I<Plerd> be aware of these changes:

=over 5

=item The last release to CPAN by the original author was in December of 2019.
...
package doesn't work, B<it does and works well!> The build however may
be busted. So...

=item The build has changed.

The old build apparently used travis-ci and may not even be working
correctly at this point.  This version uses my own L<home grown
...
hack on if you wish.  The C<Makefile.PL> that my tools produce should
be sufficient to keep you building.

=item I've refactored some modules to conform to Perl Best Practices.

Seems I have a hard time dealing with all of the I<flycheck> issues
nagging at me when I open these modules in my C<emacs>
...
=item You can now add a new attribute (C<image_style>) to your blog.
...
E<lt>img style="width: 20%; height: 20%;" src="http://blog.tbcdevelopmentgroup.com/img/bigfoot.png"E<gt>

=item The markdown engine has been changed to L<Text::Markdown::Discount>.

The old version used L<Text::MultiMarkdown> a pure Perl version of a
markdown to HTML conversion utility. It suffered from a major
deficiency (IMHO) - the inability to render fenced code
blocks. Whatever other extra things C<Text::MultiMarkdown> did that
C<Text::Markdown::Discount> doesn't do are no longer supported in this
version.  So far I have not run into any limitations.  In fact, it's
faster and more reliable.

=item The remainder of this README is from the original version...
...
L<Daring Fireball|http://daringfireball.net/archive>), and syndication
documents in Atom and L<JSON Feed|http://jsonfeed.org> formats. All
these are constructed from simple, customizable templates.

That's it! That's all Plerd does.

If you have the time and inclination, you may watch L<a 20-minute
...
stay informed of Plerd changes, consider joining L<the plerd-announce
...
B<First, make sure you have the C<cpanm> program on your machine.> It
...
B<To install Plerd from source>, set the current working directory to the same directory containing this README file, make sure you have C<cpanm> as described above, and then do this:
...
Run C<plerdall --init> to create a new directory called C<plerd/> in
...
file in C<plerd/plerd.conf>.

If you'd like the directory named something else or located somewhere
else, you can provide it as an argument, e.g. C<plerdall
--init=/some/other/location>. See the C<plerdall> man page for full
details.

Here's the purpose of the subdirectories that C<plerdall --init>
creates:
...
C<templates>: Holds your blog's templates. C<plerdall --init> will
place a full complement of sample templates in this directory for you.


=item *

C<docroot>: Will hold your blog's actual docroot, ready for serving up
by the webserver software of your choice.

=item *

C<conf>: Holds your blog's configuration files. C<plerdall --init>
will place an example C<plerd.conf> file in this directory for you.
...
wish (a C<drafts> folder, perhaps?). Plerd will happily ignore them.

=item 2.

Edit the C<plerd.conf> file in the directory that you created in the
previous step to best suit your needs. The file itself is extensively
commented and self-documenting.

You can optionally move or copy the file to C<.plerd> in your home
...
As noted above, the C<templates> directory that you created in the
first step of this process contains sample templates that you can
customize as much as you'd like. They are rendered using L<Template
...
main temlates will invoke via L<the [% INCLUDE %]
directive|http://www.template-toolkit.org/docs/manual/Directives.html#section_INCLUDE>,
and so on.

=item 4.

Configure the webserver of your choice such that it treats the
C<docroot> subdirectory (which you created as part of the first step)
...
B<plerdall> creates a new website in Plerd's docroot directory, based
...
C<--init> feature referred to above.)
...
B<plerdwatcher> runs a daemon that monitors the Dropbox-synced source
directory for changes, republishing files as necessary.

B<I<This is where the magic happens.>> While both I<plerdwatcher> and
...
It also accepts the verbs C<stop>, C<restart>, and C<status>, as well
as L<all the command-line options listed in the App::Daemon
...
somewhere I<outside of your blog's source directory>. (Recall that any
...
your entry say C<title: [whatever]>, followed by two newlines,
followed in turn by the body of your post.

For example, a valid, ready-to-publish source file could be called
C<today.markdown>, and it could contain this, in full:
...
If you manually give your post a C<time:> timestamp, and it's in W3C
date-time format, then Plerd will leave that timestamp alone.


=item *

If you leave the timestamp out, I<and> include in your post's filename
a date of yesterday or earlier
(e.g. C<1994-06-10-i-like-ace-of-base.md>), then Plerd will set the
...
from the C<archive.html> page. It will also appear in the recent-posts
...
editing files, I<no matter what computer you're using>, so long as it
...
updates to your blog by hitting I<File E<rarr> Save> in your text
...
your blog, please L<see the Plerd wiki on
...
named C<tags>, then Plerd will add the post to those linked from a
file named C<tags/[tag].html>, relative to the blog's docroot. It will
also link to that page form C<tags/index.html>.
...
I<Mind your capitalization with tags!> If faced with inconsistent
...
them from your templates via a hash named C<attributes> attached to
every post object. For example, if a post's metadata looks like this:

    title: Example of user-defined attributes
    byline: Sam Handwich

Then you can refer to C<post.attributes.byline> to fetch that value
from within the C<post.tt> template file, even though "byline" is not
...
file, you can direct Plerd to add L<Open Graph|http://ogp.me> and
L<Twitter Card|https://dev.twitter.com/cards/overview>-enabling
...
B<twitter_id>: If present, then Plerd will try to attach Twitter Card
...
B<image>: If present, Plerd will use this URL as the location of a
default image to use in the metadata for any post that doesn't define
its own I<image> attribute.

If I<not> present, Plerd will I<not> generate any social-media
metadata for any post lacking an I<image> attribute.



=item *

B<image_alt>: A textual description of the image referenced by the
C<image> attribute. (Equivalent in usage to the "alt" attribute in an
HTML C<<< <img> >>> tag.) Plerd will just leave this blank, if you
...
B<image>: The URL of an image to associate with this post within
social-media links. (This could refer to image that also appears in
your post by way of an HTML C<<< <img> >>> tag, but it doesn't have
to.)

If not defined, then Plerd will instead use the blog's I<image>
configuration directive. If I<that> is also undefined, then Plerd will
...
Plerd supports
L<MultiMarkdown|https://fletcherpenney.net/multimarkdown/> syntax out
of the box! Go ahead and put MultiMarkdown tables and stuff into your
posts. it'll just work.

=head3 Webmention

Plerd supports L<Webmention|https://jmac.org/webmention/>, an open
...
the linking page employs L<Microformats2|http://microformats.io>
...
the C<plerdwatcher> and C<plerdall> programs for details about
...
L<Fogknife|http://fogknife.com>, for which it was written.
...
L<useiconic.com|https://thenounproject.com/useiconic.com>. Both are
shared through a L<Creative Commons Attribution 3.0 United
States|https://creativecommons.org/licenses/by/3.0/us/> license, and
come to this project via L<The Noun
