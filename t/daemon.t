...
my $good_model_file  = Path::Class::File->new( $model_dir,  'good-source-file.md', );
my $good_source_file = Path::Class::File->new( $source_dir, 'good-source-file.md', );
$good_model_file->copy_to($good_source_file);

chmod oct('0644'), "$source_dir/good-source-file.md";
...
my $run_dir_path = Path::Class::Dir->new( $FindBin::Bin, 'run', );
my $log_dir_path = Path::Class::Dir->new( $FindBin::Bin, 'log', );
...
system( $^X, '-I', "$FindBin::Bin/../lib/", $daemon, "--config=$config_file_path", 'start', );
