...
my $blog_dir = Path::Class::Dir->new("$bin_dir/testblog");
$blog_dir->rmtree;
...
my $model_dir = Path::Class::Dir->new("$dist_dir/t/source_model");

foreach ( Path::Class::Dir->new("$dist_dir/t/source_model")->children ) {
  my $filename = $_->basename;
  $filename =~ s/TODAY/$ymd/;
  my $destination = Path::Class::File->new( $source_dir, $filename );
  $_->copy_to($destination);
  chmod oct('0644'), "$source_dir/$filename";
}
...
is( scalar( $docroot_dir->children ), $expected_docroot_count, "Correct number of files generated in docroot." );
...
### Test formatting in titles and filenames
{
  ...
  like( $post, qr{an <em>example</em> of a “backdated}, 'Post title is formatted.' );
  my $json = Path::Class::File->new( $docroot_dir, 'feed.json' )->slurp;
  like( $json, qr{A good source file}, 'JSON Feed file has HTML-stripped titles.', );
}

### Test published-file naming
{
  my $renamed_file     = Path::Class::File->new( $docroot_dir, $ymd . '-a-good-source-file.html' );
  my $not_renamed_file = Path::Class::File->new( $docroot_dir, '1999-01-01-backdated.html' );
  ...
  my $renamed_file_with_funky_title
    = Path::Class::File->new( $docroot_dir, $ymd . '-apostrophes-and-html-shouldnt-turn-into-garbage.html', );
  is( -e $renamed_file_with_funky_title, 1, 'Source file with formatted title received a nice clean published filename.' );
}

### Make sure tag pages look as expected
{
  my $tag_index_file  = Path::Class::File->new( $docroot_dir, 'tags', 'index.html' );
  my $tag_detail_file = Path::Class::File->new( $docroot_dir, 'tags', 'bar with spaces.html' );
  ...
  like( $tag_index_content, qr{<h1>All Tags.*<li>.*<li>.*</ul>.*sidebar"}s, "The tag-index page links to two tags.", );
}
...
Oh no, this post has a tag whose case conflicts with an existing one.
  ...
  $plerd->publish_all;
  warning_like { $plerd->publish_all } qr/conflicts/, 'Tag case-conflicts generate a warning';
  unlink $naughty_file;
}
...
### Make sure re-titling posts works as expected
{
  ...
  my $welcome_file   = Path::Class::File->new( $docroot_dir, $ymd . '-a-good-source-file.html', );
  my $unwelcome_file = Path::Class::File->new( $docroot_dir, $ymd . '-a-retitled-source-file.html', );

  is( $docroot_dir->contains($welcome_file), 1, 'A file named after the old title is still there.', );
  isnt( $docroot_dir->contains($unwelcome_file), 1, 'A file named after the new title is not there.', );

  $text =~ s/-a-good-source-file/-a-retitled-source-file/;
  $source_file->spew($text);

  $plerd->publish_all;
  is( $docroot_dir->contains($unwelcome_file), 1, 'A file named after the new title is there now.', );

### Test GUIDs
  $plerd->publish_all;
  like( $source_file->slurp, qr/guid: /, 'Source file contains a GUID, as expected.', );

### Make sure descriptions work in different cases.
  is(
    $plerd->post_with_url("http://blog.example.com/$ymd-metatags.html")->description,
    'Fun with social metatags.',
    'Manually-set post description works.',
  );
  like(
    $plerd->post_with_url("http://blog.example.com/$ymd-metatags-with-image.html")->description,
    qr/This file sets up some attributes/,
    'Automatically derived description works.',
  );

  like(
    $plerd->post_with_url("http://blog.example.com/$ymd-metatags-with-image-and-alt.html")->description,
    qr/This file, which is awesome\s*, sets up some attributes/,
    'Automatically derived description works, with leading image tag present.',
  );

  # make sure that multimarkdown tables work
  like(
    $plerd->post_with_url("http://blog.example.com/$ymd-markdown-table.html")->body,
    qr{<td>\s*Pizza\s*</td>}, 'Markdown tables are rendered.',
  );

### Test miscellaneous-attribute pass-through
  # We need to edit the post template so it'll do something with a received
  # pass-through attribute.
  my $post_template         = Path::Class::File->new( $blog_dir, 'templates', 'post.tt', );
  my $post_template_content = $post_template->slurp;
  $post_template_content =~ s{<div class="body e-content">}
    {<div class="byline">[% post.attributes.byline %]</div><div class="body e-content">};
  $post_template->spew($post_template_content);
  $plerd->publish_all;

  my $byline_post = Path::Class::File->new( $docroot_dir, '2000-01-01-this-post-has-extra-headers.html', );

  like( $byline_post->slurp, qr/"byline">Sam Handwich/, 'Miscellaneous header passed through to the template', );

### Test newer / older post links
  # (Including robustness after new posts are added)
  my $new_post = Path::Class::File->new( $source_dir, 'another_post.md', );
  ...
  is( $first_post->older_post,  $second_post,      'First post has correct older post.' );
  is( $second_post->newer_post, $first_post,       'Second post has correct newer post.' );
  is( $second_post->older_post, $third_post,       'Second post has correct older post.' );
  is( $last_post->older_post,   undef,             'Last post has no older post.' );
  is( $last_post->newer_post,   $penultimate_post, 'Last post has correct newer post.' );
...
### Test social-media metatags.
{
  ...
  like( $post,       qr{name="twitter:image" content="http://blog.example.com/logo.png"},    'Metatags: Default image', );
  like( $image_post, qr{name="twitter:image" content="http://blog.example.com/example.png"}, 'Metatags: Post image', );
  like(
    $alt_image_post,
    qr{name="twitter:image:alt" content="A lovely bunch of coconuts."},
    'Metatags: Post-specific alt-text',
  );
  like( $image_post, qr{name="twitter:image:alt" content=""}, 'Metatags: Empty default alt-text', );

  like( $post,       qr{name="twitter:card" content="summary"},             'Metatags: Default image is a thumbnail', );
  like( $image_post, qr{name="twitter:card" content="summary_large_image"}, 'Metatags: Post image is full-sized', );

  like( $post, qr{name="twitter:description" content="Fun with social metatags.}, 'Metatags: Defined description', );
  ...
  like( $post, qr{name="twitter:image:alt" content="Just a test image."}, 'Metatags: Defined default alt-text', );
